...
import { setShowArticleModal } from "src/redux/slices/exploreArticles.slice";
...
const SUBSCRIPTION_TYPE = "source";

export default function SourceFollowDropdown() {
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < userFolders.length; i++) {
            ...
            let userFolderSubscription = userFolders[i].userFolderSubscriptions.find(userFolderSubscription => userFolderSubscription.sourceId === activeArticle.sourceId);
    ...
    const onClickUnfollow = async (event) => {
        setShowNewFolder(false);
        if (window.confirm(`Are you sure you want to unfollow ${activeArticle.sourceName}?`)) {
            try {
            ...
            {userFolderSubscription && (
                <>
                    <span className="text-sm">
                        <strong className="font-semibold">{activeArticle.sourceName}</strong>
                    </span>
                    <button
                        type="button"

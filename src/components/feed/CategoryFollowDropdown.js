...
const SUBSCRIPTION_TYPE = "category";

export default function CategoryFollowModal() {
    const dispatch = useDispatch();
    const location = useLocation();
    const { id } = useParams();

    const { userFolders } = useSelector((state) => state.userFolders);
    const { category } = useSelector((state) => state.showCategory);
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < userFolders.length; i++) {
            ...
            let userFolderSubscription = userFolders[i].userFolderSubscriptions.find(userFolderSubscription => userFolderSubscription.categoryId === category.id);
    ...
    const onClickUnfollow = async (event) => {
        setShowNewFolder(false);
        if (window.confirm(`Are you sure you want to unfollow ${category.name}?`)) {
            try {
